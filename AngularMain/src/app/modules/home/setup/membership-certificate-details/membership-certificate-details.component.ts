...
@Component({
  selector: 'app-membership-certificate-details',
  templateUrl: './membership-certificate-details.component.html',
  styleUrls: ['./membership-certificate-details.component.scss']
})


export class MembershipCertificateDetailsComponent implements OnInit {
    ...
    if (localStorage.getItem('AppLabels') != null) {
      ...
      for (let labels of this.AppFormLabels) {
        ...
        if (labels.formID == this.formId && labels.language == this.languageType) {
          ...
          index++;
          if(index === 1) {
            this.formHeaderLabels.push({ "headerName": "Membership Certificate", "formID": "EmployeeCertificate", "subHeaderName": "شهادة العضوية", "status": "Active"})
