...
@Component({
  selector: 'app-subscribers-deduction',
  templateUrl: './subscribers-deduction.component.html',
  styleUrls: ['./subscribers-deduction.component.scss']
})
export class SubscribersDeductionComponent implements OnInit {
  ...
  columnsToDisplay: string[] = ['empId', 'name', 'job_department',  'subscription_date', 'contract_type'];
    ...
    if (localStorage.getItem('AppLabels') != null) {
      ...
      for (let labels of this.AppFormLabels) {
        ...
        if (labels.formID == this.formId && labels.language == this.languageType) {
          ...
          index++;
          if(index === 1) {
            this.formHeaderLabels.push({"headerName": "Subscriptions Deduction", "formID": "SubscribersDeduction", "subHeaderName": "خصم الاشتراكات", "status": "Active"})
  ...
  generateSubscribersDeducationReport() {
    const filename = `Subscription_Deducation_${new Date().toLocaleDateString('en-US', { day: '2-digit', month: '2-digit', year: 'numeric' })}.pdf`;
  ...
  filterRecords(pageIndex: any) {
    ...
    this._communicationService.GetEmployeeTransactionHistoryDetails(
      this.userParams,
      this.tenentId,
      this.formData.universityId,
      this.formData.contractTypeId,
      ...
      .subscribe((response: any) => {
        ...
        this.tableData = new MatTableDataSource<any>(response.body.empDetailsWithHistoryList.map((r: any) => {
          return {
            ...
            name: r.arabicName,
            job_department: r.department_Name,
            job_desc: r.departmentDesc,
            pf_number: r.depfid,
            subscription_date: this.formatDate(r.subscribedDate),
  ...
  formatDate(subscribedDate: string): string {
    const formattedDate = this.datePipe.transform(subscribedDate, 'dd/MM/yyyy');
  ...
  exportToExcel() {
    this.excelParser.exportToExcel(this.tableData.data, "SubscribersDeducation.xlsx");
  }
