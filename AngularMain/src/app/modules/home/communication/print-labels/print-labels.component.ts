...
export class PrintLabelsComponent implements OnInit {
    ...
    if (localStorage.getItem('AppLabels') != null) {
      ...
      for (let labels of this.AppFormLabels) {
        ...
        if (labels.formID == this.formId && labels.language == this.languageType) {
          ...
          index++;
          if(index === 1) {
            this.formHeaderLabels.push({"headerName": "Print Labels", "formID": "PrintLabels", "subHeaderName": "تفاصيل تاريخ الموظف", "status": "Active"})
  ...
  generateAssemblyReport() {
    const filename = `Print_Labels_${new Date().toLocaleDateString('en-US', { day: '2-digit', month: '2-digit', year: 'numeric' })}.pdf`;

    this._communicationService.generateAssemblyReport( this.userParams,
      this.tenentId,
      ...
      this.formData.serviceTypeId).subscribe((response: HttpResponse<Blob>)  => {
      if (response.body) {
    ...
    this._communicationService.GetEmployeeHistoryDetails(
      ...
      this.tableData = new MatTableDataSource<any>(response.body.empDetailsWithHistoryList.map((r: any) => {
        return {
          ...
          name: r.arabicName,
          job_department: r.loanAmount,
          job_desc: 10,
          pf_number: 75,
          subscription_date: '11/08/2023',
          notes: 'قرض اجتماعى'
        }
      }));
    }, error => {
  ...
  exportToExcel() {
    this.excelParser.exportToExcel(this.tableData.data, "PrintLabel.xlsx");
  }
}
