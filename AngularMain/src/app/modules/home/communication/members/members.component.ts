...
export class MembersComponent implements OnInit {
    ...
    if (localStorage.getItem('AppLabels') != null) {
      ...
      for (let labels of this.AppFormLabels) {
        ...
        if (labels.formID == this.formId && labels.language == this.languageType) {
          ...
          index++;
          if(index === 1) {
            this.formHeaderLabels.push({"headerName": "Members", "formID": "members", "subHeaderName": "أعضاء", "status": "Active"})
  ...
  generateSubscribersLisReport() {
    ...
    this._communicationService.generateSubscribersLisReport(  this.userParams,
      this.tenentId,
  ...
  exportToExcel() {
    this.excelParser.exportToExcel(this.tableData.data, "Members.xlsx");
  }
}
